...
struct ImmersiveDiceView: View {
        ...
        RealityView { content in
               ...
               let environment = try? await EnvironmentResource(named: "studio")
            {
                ...
                dice.components[PhysicsBodyComponent.self] = .init(PhysicsBodyComponent(
                    massProperties: .default,
                    material: PhysicsMaterialResource.generate(staticFriction: 0.8, dynamicFriction: 0.5, restitution: 0.08),
                    mode: .dynamic
                ))
    ...
    var dragGesture: some Gesture {
        DragGesture()
            .targetedToAnyEntity()
            .onChanged {
                value in value.entity.position = value.convert(value.location3D, from: .local, to: value.entity.parent!)
                value.entity.components[PhysicsBodyComponent.self]?.mode = .kinematic
            }
            .onEnded {
                value in value.entity.components[PhysicsBodyComponent.self]?.mode = .dynamic
